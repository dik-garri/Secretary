...
Реестр инструментов `TOOLS_` в `Actions.gs` — единственный источник правды
(имя, описание, схема параметров, handler). Список возможностей в промпте
Gemini и валидация ответа генерируются из него автоматически.

1. Добавьте запись в `TOOLS_` (name, desc, params, handler).
2. Напишите handler `(user, intent) → строка ответа` там же.

Всё. Модель не может увидеть intent, которого нет в коде, и наоборот.
Существующие механизмы при добавлении не затрагиваются.
