...
## Задачи, заметки, сводки

- **Задачи** с приоритетами (🔴 HIGH / NORMAL / 🔵 LOW) и сроками:
  «добавь важную задачу: отчёт до пятницы», «задача про отчёт — срочная»,
  «удали задачу про…», «все задачи выполнены».
- **Заметки** (лист Notes, создаётся автоматически): «запиши: …»,
  «что я записывал про…», «покажи заметки», «удали заметку про…».
- **Сводки**: по запросу («что у меня сегодня / на неделю») и по расписанию
  («присылай сводку каждый день в 8», «еженедельную по понедельникам в 9»,
  «отключи ежедневную сводку»). Расписание хранится в колонке Settings листа
  Users (JSON: `{"daily":"08:00","weekly":"MON 08:00"}`), доставка идёт через
  существующий триггер `checkReminders` — отдельный триггер не нужен.
